...
pw.Widget PDFTemplateObat(List<StokMasukObat>? obatMasuk,
    List<StokKeluarObat>? obatKeluar, List<Obat>? stok) {
...
Future<void> printReportObat(BuildContext context) async {
  final pdf = pw.Document();
  final stokObatMasuk = await fetchLaporanMasukStokObat();
  final stokObatKeluar = await fetchLaporanKeluarStokObat();
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (pw.Context context) {
        return [PDFTemplateObat(stokObatMasuk, stokObatKeluar, daftarStok)];
      },
    ),
  );
  ...
  final storedDirectory = await FilesystemPicker.open(
      ...
      pickText:
          'Simpan dokumen disini sebagai laporan_dokumen_obat (${dtf.format(DateTime.now())}).pdf');

  if (storedDirectory != null) {
    DateFormat dtf = DateFormat("dd MMMM yyyy");
    final file = File(
        "${storedDirectory}/laporan_dokumen_obat (${dtf.format(DateTime.now())}).pdf");
